...
def characteristic_exponent_nov(omega, Delta, sigma, lambda0, alpha, beta, mu, rho):
    # Compute the characteristic exponent of the novel process-driven jump-diffusion model
...
def characteristic_function_nov(omega, S0, m, n, Delta, sigma, lambda0, alpha, beta, mu, rho):
    # Compute the characteristic function of the log-price under the novel process-driven jump-diffusion model
    return np.exp(1j * omega * (np.log(S0) + m * Delta * n / 2) + np.sum([characteristic_exponent_nov(omega * (n - k + 1) / (n + 1), Delta, sigma, lambda0, alpha, beta, mu, rho) for k in range(1, n+1)], axis=0))
...
def GEOMETRIC_price_fft_nov(S0, K, T, r, q, sigma, lambda0, alpha, beta, mu, rho, option_type, n, damping, N, eta):
    ...
    m = (r - q) - characteristic_exponent_nov(-1j, Delta, sigma, lambda0, alpha, beta, mu, rho) / Delta

    # STEP 4: ADJUSTED CHARACTERISTIC FUNCTION WITH DAMPING
    def adjusted_characteristic_func(frequency):
        cf = characteristic_function_nov(frequency - (damping + 1) * 1j, S0, m, n, Delta, sigma, lambda0, alpha, beta, mu, rho)
