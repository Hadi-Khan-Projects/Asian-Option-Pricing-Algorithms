...
def characteristic_exponent_merton(omega, Delta, sigma, lam, alpha, delta):
    return -0.5 * sigma**2 * omega**2 * Delta + lam * Delta * (np.exp(1j * omega * alpha - 0.5 * omega**2 * delta**2) - 1)

def characteristic_function_merton(omega, S0, m, n, Delta, sigma, lam, alpha, delta):
    return np.exp(1j * omega * (np.log(S0) + m * Delta * n / 2) + np.sum([characteristic_exponent_merton(omega * (n - k + 1) / (n + 1), Delta, sigma, lam, alpha, delta) for k in range(1, n+1)], axis=0))

def GEOMETRIC_price_fft_jd(S0, K, T, r, q, sigma, lam, alpha, delta, option_type, n, damping, N, eta):
    ...
    m = (r - q) - characteristic_exponent_merton(-1.0j, Delta, sigma, lam, alpha, delta) / Delta

    # STEP 4: ADJUSTED CHARACTERISTIC FUNCTION WITH DAMPING
    def adjusted_characteristic_func(frequency):
        cf = characteristic_function_merton(frequency - (damping + 1) * 1j, S0, m, n, Delta, sigma, lam, alpha, delta)
